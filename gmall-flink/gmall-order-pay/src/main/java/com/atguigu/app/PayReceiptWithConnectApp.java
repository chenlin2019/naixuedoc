...
public class PayReceiptWithConnectApp {
    public static void main(String[] args) throws Exception {
        ...
        //3.按照事务Id分组之后进行连接
        ConnectedStreams<OrderEvent, ReceiptEvent> connectDS =
                orderEventDS.keyBy(OrderEvent::getTxId).connect(receiptEventDS.keyBy(ReceiptEvent::getTxId));

        //4.使用ProcessFunction处理2个流的数据
        SingleOutputStreamOperator<String> result = connectDS.process(new PayReceiptKeyedProcessFunc());
    ...
    public static class PayReceiptKeyedProcessFunc extends KeyedCoProcessFunction<String, OrderEvent, ReceiptEvent,
            String> {
        ...
        public void open(Configuration parameters) throws Exception {
            //状态初始化
            orderEventState = getRuntimeContext().getState(new ValueStateDescriptor<OrderEvent>("order-state",
                    OrderEvent.class));
            receiptEventState = getRuntimeContext().getState(new ValueStateDescriptor<ReceiptEvent>("receipt-state",
                    ReceiptEvent.class));
