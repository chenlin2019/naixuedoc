...
public class HotItemAppExer {
    public static void main(String[] args) throws Exception {
        ...
        //3.商品id分组
        KeyedStream<UserBehavior, Long> keyedDS = userBehaviorDS.keyBy(UserBehavior::getItemId);

        //4.开窗
        WindowedStream<UserBehavior, Long, TimeWindow> windowedStream = keyedDS.timeWindow(Time.hours(1), Time.minutes(5));

        //5.聚合，逐条计算每个窗口商品点击次数，获取窗口信息
        SingleOutputStreamOperator<ItemCount> itemCountWindowEndDS = windowedStream.aggregate(new ItemCountAppFunction(), new ItemCountWindowFunction());

        //6.再以窗口关闭时间分组
        KeyedStream<ItemCount, Long> itemCountKeyedStream = itemCountWindowEndDS.keyBy(ItemCount::getWindowEnd);

        //7.使用ProcessFunction实现收集每个窗口的数据，做排序，输出(状态编程，定时器)
        SingleOutputStreamOperator<String> result = itemCountKeyedStream.process(new ItemKeyedProcessFunction(5));
    ...
    public static class ItemCountAppFunction implements AggregateFunction<UserBehavior, Long, Long> {
    ...
    public static class ItemCountWindowFunction implements WindowFunction<Long, ItemCount, Long, TimeWindow> {
    ...
    public static class ItemKeyedProcessFunction extends KeyedProcessFunction<Long, ItemCount, String> {

        //定义属性
        private long TopN;

        public ItemKeyedProcessFunction() {
        }

        public ItemKeyedProcessFunction(long topN) {
            TopN = topN;
        }
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            sb.append("==============").append(new Timestamp(timestamp - 1000)).append("==============").append("\n");
