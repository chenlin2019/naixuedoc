...
public class HotUrlApp {
    public static void main(String[] args) throws Exception {
        ...
        //2.读取文本数据，转换为JavaBean，过滤，提取数据中的时间戳生成watermark
        ...
        //3.按照URL分组
        KeyedStream<ApacheLog, String> urlkeyedDS = apacheLogDS.keyBy(ApacheLog::getUrl);
        //4.开滑动窗口，窗口大小10分钟，滑动步长5秒钟，允许处理1分钟的迟到的数据
        WindowedStream<ApacheLog, String, TimeWindow> windowedStream = urlkeyedDS.timeWindow(Time.minutes(10), Time.seconds(5))
                .allowedLateness(Time.minutes(1));
        //5.计算每个窗口每部每个URL的访问次数，滚动聚合，用windowFunction提取窗口信息
        SingleOutputStreamOperator<UrlCount> urlCountWithWindowDS = windowedStream.aggregate(new UrlCountAggFunc(), new UrlCountWindowFunc());

        //6.按照窗口信息分组
        KeyedStream<UrlCount, Long> urlCountLongKeyedStream = urlCountWithWindowDS.keyBy(UrlCount::getWindowEnd);
